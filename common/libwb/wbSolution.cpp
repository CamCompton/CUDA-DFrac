...
wbBool wbSolution(char *expectedOutputFile, char *outputFile, char *type0,
  ...
  if (outputFile != NULL) {
    if (wbString_sameQ(type, "image")) {
      wbImage_t img        = wbImage_new(columns, rows, depth);
      /*"data" is the generic data structure, so here data contains the image 
       *which we need to then get the raw data from*/ 
      memcpy(wbImage_getData(img), wbImage_getData((wbImage_t)data),
